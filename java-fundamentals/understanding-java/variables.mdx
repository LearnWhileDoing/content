...
Those 8 primitive data types are *primitives*, meaning they are not *objects*. Furthermore, this means that you cannot directly call [methods](basic-java/methods) on them, but must instead use helper classes instead.