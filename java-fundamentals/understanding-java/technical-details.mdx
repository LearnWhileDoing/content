...
1. [Inheritance](advanced/inheritance), the sharing of information
2. [Abstraction](advanced/abstraction), the hiding of information
3. [​Encapsulation](basic-java/methods), the grouping of information
4. [Polymorphism](advanced/abstraction), the redefining of information
...
When dealing with computer applications and programming, you must be cognizant of the amount of *memory* your program uses. The more memory your program uses, the slower it will run, and the less efficient it will be. Java, like many languages today, manages the memory your program uses automatically through use of a *garbage collector*. It collects the memory used by objects that are no longer being used by the program, and opens that memory back up to the program.
