...
You can view the code used in this lesson at [https://dartpad.dev/91106c1fd8829ce1e3c348ff83bcd9e6](https://www.notion.so/91106c1fd8829ce1e3c348ff83bcd9e6)
...
The above is an example of an `if` statement code block. It will only run if the given [condition](notion://www.notion.so/basic-java/programming-concepts/booleans#logical-expressions) is met. In this case, if `shouldMove` is `true`, then we call `move()` on the robot object.

We can also execute code if the condition is *not* met.
...
What if there are different conditions that we want to compare? When there are more than one outcome of a section of control flow in your code, it is considered to have *multiple branches* because there are multiple ways the control flow could end (kind of like a choose-your-own-adventure).
...
As you can see, there are 3 parts to a ternary statement. The first part, the *condition*, accepts a boolean condition. If the boolean condition is `true`, the ternary expression will result with the first outcome. Otherwise, the ternary expression will result with the second outcome.

Note that you can only use one condition in a ternary expression. If you have multiple branches, you *can* nest ternary expressions, but it is advised that you use if/else-if/else or switch statements instead.

<Challenge>

<ChallengeDescription>

Can you add more cases to this `switch` statement that make the light turn different colors?
