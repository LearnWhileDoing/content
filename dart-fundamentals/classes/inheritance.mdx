...
Later on, we discover a shape known as a `Rectangle`. Its name is "Rectangle", it has its own color, but it also has a `width`and a `height`. A rectangle ***is-a*** shape, so we will *inherit* the `Shape` class.
...
A `Rectangle` ***is-a*** `Shape`. Therefore, we extended the `Shape` class and inherit its properties. If an object ***has-a*** object, we would make it an instance variable!
